...
public struct SettingsPanel: View {
  ...
  public init(selectedStyleKindRaw: Binding<String>, selectedColorSchemeRaw: Binding<String>) {
    ...
    print(
      "[SettingsPanel] init: styleKindRaw=\(selectedStyleKindRaw.wrappedValue), colorSchemeRaw=\(selectedColorSchemeRaw.wrappedValue)"
    )
    logger.info(
      "[SettingsPanel] init: styleKindRaw=\(selectedStyleKindRaw.wrappedValue), colorSchemeRaw=\(selectedColorSchemeRaw.wrappedValue)"
    )
  ...
  public var body: some View {
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 24) {
          ...
          VStack(spacing: 12) {
            ModelCardView(
              model: .init(
                ...
                statusMessage: "Ready", statusColor: currentStyle.successColor)
            )
            .frame(width: 220, height: 120)
            .uiaiStyle(currentStyle)
            ErrorBanner(
              message: "This is a preview error banner.", style: .error,
              isPresented: $showErrorPreview
            )
            .uiaiStyle(currentStyle)
          }
          .padding(.vertical, 8)
          ...
          OnboardingBanner(
            ...
            message: "Configure your model, generation, and app preferences here."
          )
          .padding(.bottom, 8)
      ...
      .onAppear {
        print(
          "[SettingsPanel] body: styleKindRaw=\(selectedStyleKindRaw), colorSchemeRaw=\(selectedColorSchemeRaw)"
        )
        logger.info(
          "[SettingsPanel] body: styleKindRaw=\(selectedStyleKindRaw), colorSchemeRaw=\(selectedColorSchemeRaw)"
        )
