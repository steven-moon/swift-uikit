...
public struct ModelDiscoveryView: View {
      ...
      if #available(macOS 13, *) {
                ...
                if isLoading {
                  ...
                  if huggingFaceToken.isEmpty {
                    ...
                    .onAppear {
                      ...
                      if !huggingFaceToken.isEmpty {
                        Task { await validateToken(token: huggingFaceToken) }
                      }
                    }
                    Divider()
                  }
        ...
        .task {
          AppLogger.shared.debug(
            "ModelDiscoveryView", ".task triggered, loading models and recommended models")
          loadModels()
  ...
  private func loadModels() {
    ...
    Task {
      do {
        ...
        await MainActor.run {
          AppLogger.shared.debug(
            "ModelDiscoveryView", "Models loaded successfully. Count: \(results.count)")
          self.models = results
        ...
        await MainActor.run {
          AppLogger.shared.error(
            "ModelDiscoveryView", "Error loading models: \(error.localizedDescription)")
  ...
  private func loadRecommendedModel() {
    ...
    Task {
      do {
        ...
        await MainActor.run {
          AppLogger.shared.debug(
            "ModelDiscoveryView", "Recommended models loaded. Count: \(models.count)")
          recommendedModels = models
        }
      } catch {
        AppLogger.shared.error(
          "ModelDiscoveryView", "Error loading recommended models: \(error.localizedDescription)")
  ...
  private func modelHealthStatus(_ model: ModelSummary) -> (
    icon: String, color: Color, message: String
  ) {
  ...
  private var filteredModels: [ModelSummary] {
    let filtered = models.filter { model in
      ...
      let quantMatch =
        selectedQuant == "All" || (model.quantization?.lowercased() == selectedQuant.lowercased())
    ...
    case "Model Size":
      return filtered.sorted {
        (Double($0.parameters?.replacingOccurrences(of: "B", with: "") ?? "0") ?? 0)
          > (Double($1.parameters?.replacingOccurrences(of: "B", with: "") ?? "0") ?? 0)
      }
    case "Name (A-Z)":
      return filtered.sorted {
  ...
  private func modelType(for architecture: String?) -> String? {
    ...
    if arch.contains("llama") || arch.contains("qwen") || arch.contains("mistral")
      || arch.contains("phi") || arch.contains("gemma")
    {
      return "LLM"
    }
    return nil
...
private struct ModelDownloader {
  func downloadModel(_ config: ModelConfiguration, progress: @escaping (Double) -> Void)
    async throws -> Bool
  { return true }
}
private struct FileManagerService {
  ...
  func getModelPath(modelId: String) async throws -> URL {
    throw NSError(domain: "NotImplemented", code: 0)
  }
}
private struct HuggingFaceAPI {
...
private struct ModelDiscoveryService {
  static func searchMLXModels(query: String, limit: Int) async throws -> [ModelSummary] {
    return []
  }
  static func recommendedMLXModelsForCurrentDevice(limit: Int) async throws -> [ModelSummary] {
    return []
  }
}
