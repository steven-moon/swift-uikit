...
public struct ExamplesView: View {
  ...
  private var productivityTaskList: some View {
    VStack(spacing: 12) {
      ...
      .shadow(
        color: uiaiStyle.shadow?.color ?? .clear, radius: uiaiStyle.shadow?.radius ?? 0,
        x: uiaiStyle.shadow?.x ?? 0, y: uiaiStyle.shadow?.y ?? 0)
    }
  }
  ...
  private var socialFeed: some View {
    VStack(spacing: 12) {
      socialPost(
        user: "@alice", content: "Just shipped a new SwiftUI package! 🚀", time: "2m ago",
        isLiked: true)
      socialPost(
        user: "@bob", content: "Anyone tried the new visionOS APIs?", time: "10m ago",
        isLiked: false)
      socialPost(
        user: "@carol", content: "Minimalism is the ultimate sophistication.", time: "1h ago",
        isLiked: false)
    }
    .padding(16)
    .background(uiaiStyle.backgroundColor)
  ...
  private func taskRow(_ title: String, isCompleted: Bool) -> some View {
    HStack {
      Image(systemName: isCompleted ? "checkmark.circle.fill" : "circle")
        .foregroundColor(
          isCompleted ? uiaiStyle.accentColor : uiaiStyle.foregroundColor.opacity(0.5))
  ...
  private func statCard(_ title: String, value: String, change: String, isPositive: Bool)
    -> some View
  {
    VStack(alignment: .leading, spacing: 8) {
      Text(title)
