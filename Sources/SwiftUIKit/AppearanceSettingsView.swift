...
public struct AppearanceSettingsView: View {
  ...
  private var previewStyle: any UIAIStyle {
    UIAIStyleRegistry.style(
      for: tempStyleKind ?? selectedStyleKind, colorScheme: tempColorScheme ?? selectedColorScheme)
  }
    ...
    VStack(spacing: 8) {
      ZStack(alignment: .topTrailing) {
        RoundedRectangle(cornerRadius: 14)
          .fill(
            UIAIStyleRegistry.style(for: kind, colorScheme: tempColorScheme ?? selectedColorScheme)
              .backgroundColor
          )
          .frame(width: 64, height: 48)
          .shadow(
            color: isSelected ? Color.accentColor.opacity(0.18) : .clear, radius: 6, x: 0, y: 2
          )
          .overlay(
            VStack(spacing: 0) {
              Rectangle()
                .fill(
                  UIAIStyleRegistry.style(
                    for: kind, colorScheme: tempColorScheme ?? selectedColorScheme
                  ).accentColor
                )
                .frame(height: 8)
                .cornerRadius(4)
...
extension AppearanceSettingsView {
  fileprivate var systemGroupedBackgroundColor: Color {
    #if canImport(UIKit)
  ...
  fileprivate var systemBackgroundColor: Color {
    #if canImport(UIKit)
  ...
  fileprivate var systemGray5Color: Color {
    #if canImport(UIKit)
  ...
  fileprivate var systemGray6Color: Color {
    #if canImport(UIKit)
