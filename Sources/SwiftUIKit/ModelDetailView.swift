  ...
  public var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 20) {
        ...
        HStack(spacing: 8) {
          Image(
            systemName: isCompatible ? "checkmark.shield.fill" : "exclamationmark.triangle.fill"
          )
        ...
        HStack(spacing: 8) {
          Image(systemName: health.icon)
            .foregroundColor(
              health.color == .green
                ? uiaiStyle.successColor
                : (health.color == .yellow ? uiaiStyle.warningColor : uiaiStyle.infoColor))
          Text(health.message)
            .font(.caption)
            .foregroundColor(
              health.color == .green
