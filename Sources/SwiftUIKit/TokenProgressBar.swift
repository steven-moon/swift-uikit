...
public struct TokenProgressBar: View {
    public enum Style { case normal, error, warning, success }
    ...
    private var foregroundColor: Color {
        ...
        case .success: return uiaiStyle.successColor
        }
    }
    private var backgroundColor: Color {
        ...
        case .success: return uiaiStyle.successColor.opacity(0.15)
        }
    }
}

#if DEBUG
struct TokenProgressBar_Previews: PreviewProvider {
    static var previews: some View {
        VStack(spacing: 16) {
            TokenProgressBar(progress: 0.7, label: "Downloading", style: .normal)
            TokenProgressBar(progress: 0.3, label: "Processing", style: .warning)
            TokenProgressBar(progress: 1.0, label: "Complete", style: .success)
        }
        .padding()
